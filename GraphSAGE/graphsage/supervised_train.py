...
flags.DEFINE_string('train_prefix', './GraphSAGE/example_data/cora/cora', 'prefix identifying training data. must be specified.')
...
def save_val_embeddings(sess, model, minibatch_iter, size, out_dir, mod=""):
    ...
    while not finished:
        ...
        for i, node in enumerate(val_nodes):
            if not node in seen:
                val_embeddings.append(node_outs_val[-1][i,:])
                nodes.append(node)
                seen.add(node)
...
def incremental_evaluate(sess, model, minibatch_iter, size, test=False ):
    ...
    if not os.path.exists(log_dir()):
        os.makedirs(log_dir())
    val_embeddings = np.vstack(val_embeddings)
    np.save(log_dir() + name + "-test" + ".npy",  val_embeddings)
    with open(log_dir() + name + "-test" + ".txt", "w") as fp:
        fp.write("\n".join(map(str,nodes)))
